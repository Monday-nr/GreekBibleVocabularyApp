...
func _font_size_changed(new_val: int) -> void:
	_font_size = new_val
	Global.verse_label_settings.font_size = new_val

	var data: Array[Array] = [
		["verse_font_size", "size", _font_size],
	]
	SaveLoad.save(data)

func _sl_load_config(config_file: ConfigFile) -> void:
	if config_file.has_section("verse_font_size"):
		_font_size = config_file.get_value("verse_font_size", "size")
	Global.verse_label_settings.font_size = _font_size
	text = str(_font_size)
